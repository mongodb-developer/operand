...
async function runSlowQueries(mongodb) {
  ...
  switch (pickRandomQuery) {
    case 0:
      ...
      return `performed airbnb query: ${result}`;
    case 1:
      result = await gradesQuery(mongodb);
      return `performed grades query: ${result}`;
    case 2:
      result = await gradesQuery(mongodb);
      return `performed mflix query: ${result}`;
    case 3:
      result = await gradesQuery(mongodb);
      return `performed weather data query: ${result}`;
  }
}
    
function airbnbQuery(mongodb) {
  const db = mongodb.db('RedDoorPath');
  ...
  return db.collection('listingsAndReviews')
    ...
    .sort(sort)
    .toArray()
    .then(docs => `found ${docs.length} docs`)
    .catch(err => err);
}

function gradesQuery(mongodb) {
  const db = mongodb.db('OceanInvaders');
...
function mflixQuery(mongodb) {
  const db = mongodb.db('SkyInvaders');
...
function weatherDataQuery(mongodb) {
  const db = mongodb.db('GreenDoorPath');
